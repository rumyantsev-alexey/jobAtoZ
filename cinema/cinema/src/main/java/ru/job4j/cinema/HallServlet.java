...
public class HallServlet extends HttpServlet {
    ...
    private BiConsumer<PrintWriter, JsonNode> getZone() {
        return (out, json) ->   {
                                    writeObjectToWriter(out, DB.findAllZones());
                                };
    }

    private BiConsumer<PrintWriter, JsonNode> getSession() {
        return (out, json) ->   {
                                    writeObjectToWriter(out, DB.findAllSessions());
                                };
    }

    private BiConsumer<PrintWriter, JsonNode> getBusy() {
        return (out, json) ->   {
                                    writeObjectToWriter(out, DB.returnBusySeats(json.get("calendar").textValue(), json.get("zone").textValue(),
    ...
    private BiConsumer<PrintWriter, JsonNode> getCost() {
        return (out, json) ->   {
                                    writeObjectToWriter(out, DB.costPerRow(json.get("zone").textValue(), json.get("session").textValue()));
                                };
    }
